...
You describe the complaint and upload evidence: screenshots, bills, order confirmations, PDFs.
Evidence gets read. Digital PDFs go through PyMuPDF for exact text extraction. Images go through Qwen2.5-VL, a vision-language model that reads screenshots the way a person would, picking up tables, layout, and Hindi text instead of just pixels.
Agent 1 evaluates the case. DeepSeek R1:14b assesses it across three parameters and identifies which sections of the Consumer Protection Act 2019 apply, plus a compensation estimate.
Agent 2 drafts the notice. It takes Agent 1's findings as context and writes the full formal notice in English and Hindi, which FPDF2 turns into a downloadable PDF.
...
Why a Unicode TTF font pipeline. FPDF2's built-in fonts are Latin-only. The ₹ symbol (U+20B9) crashes them outright, and Devanagari won't render at all. The app checks for a system Unicode font and falls back to downloading DejaVuSans if it doesn't find one. Anything built for Indian users needs this.
...
Prerequisites: Python 3.10+, Ollama, and a GPU with about 10GB VRAM free.
